...
class IndexedReplayBuffer(Serializable):
    def __init__(self, observation_dim, action_dim, next_observation_dim, opponent_action_dim=None, max_replay_buffer_size=10e4):
        self._Serializable__initialize(locals())

        self._max_buffer_size = int(max_replay_buffer_size)
        self._observation_dim = observation_dim
        self._next_observation_dim = next_observation_dim
        self._action_dim = action_dim
        self._opponent_action_dim = opponent_action_dim
        self.level_v_num = 2

        self._vehicle_index = np.zeros((self._max_buffer_size, ))
        
        self._observations = np.zeros((self._max_buffer_size, self._observation_dim))
        self._next_obs = np.zeros((self._max_buffer_size, self._next_observation_dim))
        ...
        self._costs = np.zeros(self._max_buffer_size)
    ...
    def add_sample(self, observation, action, reward, terminal,
                   next_observation, cost, **kwargs):
        
        self._observations[self._top] = np.squeeze(observation)
    ...
    def batch_by_indices(self, indices):
        batch = dict(
            ...
            costs=self._costs[indices].astype(np.float32),
        )
    ...
    def __setstate__(self, d):
        ...
        self._costs = np.fromstring(d['c']).reshape(self._max_buffer_size)
