...
from logger import logger, tabular
...
class MASampler(Sampler):
    def __init__(self, agent_num, leader_num, follower_num, leader_action_num = 2, max_path_length=20, min_pool_size=10e4, batch_size=64, global_reward=False, **kwargs):
        ...
        self._episode_cost = np.zeros(self.agent_num)
    ...
    def sample(self, explore=False):
        ...
        observations = th.zeros((2, self.env.num_state))
        next_observations = th.zeros((2, self.env.num_state))
        ...
        if explore:
            ...
            for i in range(self.leader_num, self.env.agent_num):      
                mix_obs = np.hstack((observations[i],  tf.one_hot(action_n[0][0], self.env.action_num)))
                supplied_observation.append(mix_obs)
        
        else:
            for i in range(self.leader_num):
                supplied_observation.append(observations[i])
                #print("obi",observations[i])
                action_n.append(self.train_agents[0].act(observations[i]))
                
            
            for i in range(self.leader_num, self.env.agent_num):
                mix_obs = np.hstack((observations[i], tf.one_hot(action_n[0][0], self.env.action_num)))
                # print("mix obs", mix_obs)
                supplied_observation.append(mix_obs)
                follower_action = self.train_agents[1].act(th.from_numpy(mix_obs))
                #print("follower act", follower_action)
        ...
        '''
        next_observation_n, reward_n, done_n, cost_n = self.env.step(action_n)
        ...
        self._episode_cost += np.array(cost_n, dtype=np.float32)
        ...
        if np.all(done_n) or self._path_length >= self._max_path_length:
            ...
            self._mean_episode_cost = self._episode_cost / self._path_length
    ...
    def log_diagnostics(self):
        for i in range(self.agent_num):
            ...
            tabular.record('mean-episode_cost_agent_{}'.format(i), self._mean_episode_cost[i])
