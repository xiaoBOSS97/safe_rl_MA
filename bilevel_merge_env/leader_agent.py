...
class LeaderAgent:
    def __init__(self,
        ...
        self._action_space = self._env_specs.action_space
    ...
    def cost_loss(self,
        ...
        # current cost
        current_cost_input = np.hstack((observations, actions_concat))
    ...
    def actor_loss(self, env, agent_id, observations, opponent_actions, weights=None):
        ...
        for action in range(policies.shape[1]):
            ...
            actions = F.one_hot(actions, env.action_num)
            actions_concat[:, :env.action_num] = actions
            
            q_values = self._qf(th.cat((observations, actions_concat), 1))
